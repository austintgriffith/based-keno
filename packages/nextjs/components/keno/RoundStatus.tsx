...
export const RoundStatus = ({
  ...
  useEffect(() => {
    if (phase === RoundPhase.Open && startBlock > 0n) {
      const endBlock = startBlock + BigInt(bettingPeriodBlocks);
      const remaining = Number(endBlock - currentBlock);
      setBlocksRemaining(Math.max(0, remaining));
    } else {
      setBlocksRemaining(0);
    }
  }, [phase, startBlock, currentBlock, bettingPeriodBlocks]);
    ...
    switch (phase) {
      ...
      case RoundPhase.Open:
        return {
          ...
          description: `~${blocksRemaining * 2}s remaining to bet`,
        };
  ...
  return (
    <div className={`rounded-xl p-4 border-2 ${phaseInfo.bgColor} ${phaseInfo.borderColor}`}>
      ...
      {/* Progress Bar for Open phase */}
      {phase === RoundPhase.Open && blocksRemaining > 0 && (
        <div className="mb-3">
          <div className="h-2 bg-base-300 rounded-full overflow-hidden">
            <div
              className="h-full bg-gradient-to-r from-success to-emerald-400 transition-all duration-1000"
              style={{ width: `${(blocksRemaining / bettingPeriodBlocks) * 100}%` }}
            />
          </div>
          <p className="text-xs text-base-content/50 mt-1 text-center">{blocksRemaining} blocks left</p>
        </div>
      )}
